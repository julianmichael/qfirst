...
@QuestionSetModel.register("sequential")
class SequentialQuestionSetModel(QuestionSetModel):
    def __init__(self,
            vocab: Vocabulary,
            slot_names: List[str],
            input_dim: int,
        ...
        super(SequentialQuestionSetModel, self).__init__(vocab, slot_names, input_dim)
    ...
    def forward(self,
                pred_reps_seq,
                slot_labels_seq: Dict[str, torch.LongTensor],
                num_questions_mask_seq: torch.LongTensor,
                **kwargs):
        # Shape: batch_size, num_questions, pred_rep_dim
        batch_size, num_questions, pred_rep_dim = pred_reps_seq.size()
        # num_questions_mask Shape: batch_size, num_questions, 1?

        # TODO batch more smartly to avoid copying with contiguous()
        # Shape: batch_size * num_questions, pred_rep_dim
        pred_reps = pred_reps_seq.contiguous().view(-1, pred_rep_dim).float()
        # Shape: batch_size * num_questions, 1
        questions_mask = num_questions_mask_seq.view(-1, 1)

        # batch_size * num_questions
        num_instances = pred_reps.size(0)

        # TODO check input_dim == pred_rep_dim

        # hidden state: start with <batch size * num_instances> start symbols
        curr_embedding = self._start_symbol.view(1, -1).expand(num_instances, -1)
        # print("curr_embedding: " + str(curr_embedding.size()))

        # ? initialize the memory cells ?
        curr_mem = []
        for l in range(self._rnn_layers):
            curr_mem.append((Variable(pred_reps.data.new().resize_(num_instances, self._dim_rnn_hidden).zero_()),
                             Variable(pred_reps.data.new().resize_(num_instances, self._dim_rnn_hidden).zero_())))
        ...
        for i, n in enumerate(self._slot_names):
            ...
            slot_logits[n] = logits.view(batch_size, num_questions, -1)

            if i < len(self._slot_names) - 1:
                # XXX converting padding so it can be embedded. do something better?
                # batch_size * num_questions
                these_slot_labels = slot_labels_seq[n].view(num_instances)
                these_slot_labels_pad_to_0 = these_slot_labels + (these_slot_labels[:] < 0).long()

                curr_embedding = self._slot_embedders[i](these_slot_labels_pad_to_0)
    ...
    @classmethod
    def from_params(cls, vocab: Vocabulary, params: Params) -> 'SequentialQuestionSetModel':
        ...
        return SequentialQuestionSetModel(vocab, slot_names, input_dim=input_dim, share_slot_hidden=share_slot_hidden, rnn_layers = rnn_layers, share_rnn_cell = share_rnn_cell, dim_rnn_hidden = dim_rnn_hidden, dim_slot_hidden = dim_slot_hidden, dim_embedding = dim_embedding, recurrent_dropout = recurrent_dropout)
