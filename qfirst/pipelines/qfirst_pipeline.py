...
class QFirstPipeline():
    def __init__(self,
                 ...
                 question_to_span_model: QuestionToSpanModel,
        ...
        self._question_model = question_model
        self._question_model_dataset_reader = question_model_dataset_reader
        self._question_to_span_model = question_to_span_model
        self._question_to_span_model_dataset_reader = question_to_span_model_dataset_reader
    ...
    def predict(self, inputs: JsonDict) -> JsonDict:
        question_model_verb_instances = list(self._question_model_dataset_reader.sentence_json_to_instances(inputs, verbs_only = True))
        question_to_span_model_verb_instances = list(self._question_to_span_model_dataset_reader.sentence_json_to_instances(inputs, verbs_only = True))
        verb_dicts = []
        for (qg_verb_instance, qa_verb_instance) in zip(question_model_verb_instances, question_to_span_model_verb_instances):
            qg_verb_instance.index_fields(self._question_model.vocab)
            ...
            for i in range(len(question_probs)):
                ...
                for slot_name in self._question_to_span_model.get_slot_names():
                    ...
                    qa_verb_instance.add_field(slot_name, slot_label_field, self._question_to_span_model.vocab)
...
def main(question_model_path: str,
    ...
    pipeline = QFirstPipeline(
        question_model = question_model_archive.model,
        question_model_dataset_reader = DatasetReader.from_params(question_model_archive.config["dataset_reader"].duplicate()),
        question_to_span_model = question_to_span_model_archive.model,
        question_to_span_model_dataset_reader = DatasetReader.from_params(question_to_span_model_archive.config["dataset_reader"].duplicate()),
