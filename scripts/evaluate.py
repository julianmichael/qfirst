...
def main(model_path: str,
    ...
    if dense_eval and out_path is None:
        print("Must specify results out path with --out for dense eval.")
        sys.exit()

    if dense_eval:
        eval_data_file = "http://qasrl.org/data/qasrl-v2/dense/dev.jsonl.gz"
        overrides = '{ "dataset_reader": { "min_answers": 6, "min_valid_answers": 0 }, "model": {"metric": {"target": "question-answer-acc-lb", "use_dense_metric": true, "recall_pegs": [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0], "save_filepath": %s } } }' % out_path
    else:
...
if __name__ == "__main__":
    ...
    main(args.model, args.dense, args.out, args.device)
