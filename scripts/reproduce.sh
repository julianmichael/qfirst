...
# train span detector
python -m allennlp.run train configs/afirst/span_detection.json --include-package qfirst -s $AFIRST_SPAN_DETECTOR_DIR
# train question generator
python -m allennlp.run train configs/afirst/question_generation.json --include-package qfirst -s $AFIRST_QUESTION_GENERATOR_DIR
...
$QFIRST_PARSER_DIR=save/afirst

# train question generator
python -m allennlp.run train configs/qfirst/question_generation.json --include-package qfirst -s $QFIRST_QUESTION_GENERATOR_DIR
# train question answerer
python -m allennlp.run train configs/qfirst/question_answering.json --include-package qfirst -s $QFIRST_QUESTION_ANSWERER_DIR
# assemble into full model
